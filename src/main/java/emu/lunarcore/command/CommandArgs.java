...
public class CommandArgs {
    ...
    public CommandArgs(Player sender, List<String> args) {
        ...
        while (it.hasNext()) {
            ...
            try {
                if (arg.length() >= 2 && !Character.isDigit(arg.charAt(0))
                        && Character.isDigit(arg.charAt(arg.length() - 1))) {
    ...
    public boolean setProperties(GameItem item) {
        ...
        if (item.getExcel().isEquipment()) {
            ...
            // if (this.getLevel() > 0) {
            // Set relic level
            item.setLevel(Math.min(15, LunarCore.getConfig().getServerOptions().maxCustomRelicLevel));
